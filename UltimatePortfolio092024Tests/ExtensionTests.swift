...
//
//  Created by Keyla Barreto on 01/14/25.
//
...
final class ExtensionTests: BaseTestCase {
    ...
    func testIssueSortingIsStable() {
        ...
        XCTAssertEqual([issue3, issue1, issue2], sorted, "Sorting issue arrays should use title then creationDate")
    }
    ...
    func testTagsActiveIssuesUnwrap() {
        let tag = Tag(context: managedObjectContext)
        let issue = Issue(context: managedObjectContext)
    ...
    func testTagSortingIsStable() {
        ...
        tag1.name = "B Tag"
        tag1.id = UUID(uuidString: "00000001-8D0D-4F8D-BFCE-023EC39D82A2")

        let tag2 = Tag(context: managedObjectContext)
        tag2.name = "B Tag"
        tag2.id = UUID(uuidString: "00000000-8D0D-4F8D-BFCE-023EC39D82A2")

        let tag3 = Tag(context: managedObjectContext)
        tag3.name = "A Tag"
        tag3.id = UUID(uuidString: "00000002-8D0D-4F8D-BFCE-023EC39D82A2")

        let allTags = [tag1, tag2, tag3]
        let sorted = allTags.sorted()

        XCTAssertEqual([tag3, tag2, tag1], sorted, "Sorting tag arrays should use name then UUID string")
    }
